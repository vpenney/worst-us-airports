...
},{}],"data/departure_locations_2018.csv":[function(require,module,exports) {
module.exports = "/departure_locations_2018.e416fc96.csv";
...
var colorScale = d3.scaleSequential(d3.interpolateYlOrRd).clamp(true);

var radiusScale = d3.scaleLinear().range([1.5, 3]);
// .clamp(true)
...
Promise.all([d3.json(require('./data/us_states.topojson')), d3.csv(require('./data/departure_locations_2018.csv'))]).then(ready);
...
function ready(_ref) {
  ...
  var delays = datapoints.map(function (d) {
    return d.delay_minutes;
  });
  var flights = datapoints.map(function (d) {
    return d.sum_flights;
  });
  ...
  d3.select('#step-map-airports').on('stepin', function () {
    console.log('Generate those airports!');
    svg.selectAll('.airports').data(datapoints).enter().append('circle').attr('d', path).attr('class', 'airports').attr('r', function (d) {
      return radiusScale(d.sum_flights);
    }).attr('transform', function (d) {
      return 'translate(' + projection([-d.Longitude, d.Latitude]) + ')';
    })
    // .attr('fill', 'white')
    .attr('fill', function (d) {
      return colorScale(d.delay_minutes);
    }).attr('opacity', 0.5);
  });

  d3.select('#step-filter-airports').on('stepin', function () {
    ...
    svg.select('airports').transition(1000);

    var circles = svg.selectAll('.airports').data(filtered);
    circles.exit().remove();
  });

  d3.select('#step-largest-airports').on('stepin', function () {
    console.log('Try that shit the other way!');

    // I only want big airports
    var largest_airports = datapoints.filter(function (d) {
      return d.sum_flights > 100000;
    });

    console.log(largest_airports);

    svg.select('airports').transition();
...
},{"d3":"../node_modules/d3/index.js","topojson":"../node_modules/topojson/index.js","./data/us_states.topojson":"data/us_states.topojson","./data/departure_locations_2018.csv":"data/departure_locations_2018.csv"}],"../../../../../.npm-global/lib/node_modules/parcel/src/builtins/hmr-runtime.js":[function(require,module,exports) {
...
//# sourceMappingURL=/02-chart.0ff011e1.map