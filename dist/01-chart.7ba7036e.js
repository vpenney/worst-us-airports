...
module.exports = "/us_states.05c8992c.topojson";
...
var nyc = [-74, 40.71];
var sf = [-122, 37];
var yng = [-80.6792, 41.2608];
var mmh = [-118.8378, 37.6242];
var hya = [-70.2803, 41.6694];
...
d3.json(require('./data/us_states.topojson')).then(ready).then(ready).catch(function (err) {
...
function ready(json) {
  ...
  svg.selectAll('.states').data(states.features).enter().append('path').attr('class', 'country').attr('d', path).attr('fill', 'darkgrey');

  d3.select('#step-yng').on('stepin', function () {
    console.log('I stepped into Ohio');

    svg.selectAll('.airports').data(yng).enter().append('circle').attr('d', path).attr('class', 'airports').attr('r', 5).attr('transform', function (d) {
      return 'translate(' + projection([-80.6792, 41.2608]) + ')';
    }).attr('fill', 'maroon').attr('opacity', 0.6);
  ...
  d3.select('#step-reset').on('stepin', function () {
    console.log('Reset map');

    // Remove any markers
    svg.selectAll('.airports').remove();

    // Reset zoom
    svg.transition().duration(750).attr('height', height + margin.top + margin.bottom).attr('width', width + margin.left + margin.right).attr('transform', 'translate(' + margin.left + ',' + margin.top + ')');
  });

  // Remove YNG marker
  d3.select('#step-reset-yng').on('stepin', function () {
    console.log('Reset map');
    svg.selectAll('.airports').remove();
  ...
  d3.select('#step-mmh').on('stepin', function () {
    console.log('Stepped into CA');

    svg.transition().attr("transform", "translate(" + [-74, 40.71] + ")scale(" + 4 + ")");

    // Add MMH marker
    svg.selectAll('.airports').data(mmh).enter().append('circle').attr('d', path).attr('class', 'airports').attr('r', 5).attr('transform', function (d) {
      return 'translate(' + projection(mmh) + ')';
    }).attr('fill', 'maroon');
  });

  // Remove MMH marker
  d3.select('#step-reset-mmh').on('stepin', function () {
    console.log('Reset map');
    svg.selectAll('.airports').remove();
  ...
  // Add a step for HYA
  d3.select('#step-hya').on('stepin', function () {
    console.log('Stepped into HYA');

    svg.transition().attr("transform", "translate(" + [-74, 40.71] + ")scale(" + 4 + ")");

    // Add HYA marker
    svg.selectAll('.airports').data(hya).enter().append('circle').attr('d', path).attr('class', 'airports').attr('r', 5).attr('transform', function (d) {
      return 'translate(' + projection(hya) + ')';
    }).attr('fill', 'maroon');
  });

  // Remove HYA marker
  d3.select('#step-reset-hya').on('stepin', function () {
    console.log('Reset map');
    svg.selectAll('.airports').remove();
...
},{"d3":"../node_modules/d3/index.js","topojson":"../node_modules/topojson/index.js","./data/us_states.topojson":"data/us_states.topojson"}],"../../../../../.npm-global/lib/node_modules/parcel/src/builtins/hmr-runtime.js":[function(require,module,exports) {
...
//# sourceMappingURL=/01-chart.7ba7036e.map